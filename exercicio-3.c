...
void freq(int v[],int n){	//FUNÇÃO PRA CALCULO DO ELEMENTO QUE APARECE MAIS VEZES
	...
	for(i=0;i<n;i++){		//PEGA UM ELEMENTO DO VETOR PARA COMPARAR A IGUALDADE COM OUTROS
		...
		for(j=0;j<n;j++){	//FAZ A COMPARAÇÃO PARA VER A QUANTOS O ELEMENTO i É IGUAL
			if(v[i]-v[j]==0)f++;
			if(f>a){
			a=f;			//SE NOSSO V[i] TIVER MAIS REPETIÇÕES QUE O VALOR ARMAZENADO ANTERIORMENTE
			rep[0]=v[i];	//ELE SE TORNA O ELEMENTO COM MAIS REPETIÇÕES
			}		
			if(f==a&&rep[m]!=v[i]){		//SE O NOVO ELEMENTO COMPARADO TIVER O MESMO NUMERO DE REPETIÇOES QUE O ORIGINAL
